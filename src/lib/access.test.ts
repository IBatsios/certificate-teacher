...
describe("decideAccess for a student", () => {
  test("lets them open the test page and its results", () => {
    // Arrange
    const role = "student";

    // Act
    const decisions = ["/test", "/test/anything"].map((path) =>
      decideAccess(path, role),
    );
  ...
  test("sends them from sign-in to the test page, their home", () => {
    // Arrange
    const role = "student";
    ...
    // Assert
    expect(decision).toEqual({ kind: "redirect", to: "/test" });
  });
});
...
describe("decideAccess for the admin", () => {
  ...
  test("forbids the test page, which is for students", () => {
    // Arrange
    const role = "admin";
