...
2. Variables on the app service, each one from `.env.example`: `DATABASE_URL` as a reference to the Postgres service's own variable, a fresh `AUTH_SECRET` from `openssl rand -base64 32`, `AUTH_URL=https://teacher.ioannisbatsios.com`, `AUTH_TRUST_HOST=true`, `ORIGIN_SECRET` from `openssl rand -base64 32`, `AUTH_RESEND_KEY` and `EMAIL_FROM` for email, and `ADMIN_EMAIL`. The app refuses to start when `AUTH_TRUST_HOST` is set without `AUTH_URL` and `ORIGIN_SECRET`, or when the email transport it picked is missing a variable. Node 22 comes from `.node-version`. Never set `RAILPACK_PRUNE_DEPS`: the pre-deploy command needs the Prisma CLI, which is a dev dependency.
...
6. Email: a Resend API key in `AUTH_RESEND_KEY`, and an `EMAIL_FROM` on a domain Resend has verified, so magic links arrive. Setting the key switches magic-link email to Resend's HTTPS API; `EMAIL_SERVER` is then ignored (D66). Do not use SMTP on Railway: it is disabled below the Pro plan, and the send hangs for two minutes before failing.
7. At Cloudflare, the DNS records that let the sending domain past a spam filter. Resend's dashboard lists the DKIM and SPF records to add; put them in unproxied (DNS only). Then add DMARC by hand, which Resend does not ask for and Gmail does look at: a TXT record on `_dmarc.send.ioannisbatsios.com` with value `v=DMARC1; p=none`. Without it the first emails from a new sending domain land in spam (D67). `p=none` asks for no enforcement; tighten to `quarantine` once you trust the setup. An optional `rua=mailto:` for aggregate reports must use an address on `ioannisbatsios.com`, not a Gmail or other outside address: DMARC only sends reports across organizational domains when the receiving domain publishes an authorization record, which the big mailbox providers do not, so reports to an outside address are silently dropped. Check the result at https://www.mail-tester.com before telling a student to sign up.
8. The daily backup schedule on the Postgres service.
9. The app keeps sign-in limits in memory, which is right for one instance. If the service is ever scaled to several, move them to a shared store first (D39).
