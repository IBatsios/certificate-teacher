...
7. At Cloudflare, the DNS records that let the sending domain past a spam filter. Resend's dashboard lists the DKIM and SPF records to add; put them in unproxied (DNS only). Then add DMARC by hand, which Resend does not ask for and Gmail does look at: a TXT record on `_dmarc.send.ioannisbatsios.com` with value `v=DMARC1; p=none; rua=mailto:you@example.com`. Without it the first emails from a new sending domain land in spam (D67). `p=none` only asks for reports; tighten to `quarantine` once the reports are clean. Check the result at https://www.mail-tester.com before telling a student to sign up.
