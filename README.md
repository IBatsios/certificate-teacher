...
The app runs on Railway at https://teacher.ioannisbatsios.com from the `main` branch. `railway.json` in this repo tells Railway how: build with Railpack, start with `pnpm start`, run `pnpm prisma migrate deploy` as the pre-deploy command before the new version takes traffic, and treat the home page as the health check.
...
1. Railway project with two services: Postgres, and the app connected to this GitHub repository on branch `main` so merges deploy.
2. Variables on the app service, each one from `.env.example`: `DATABASE_URL` as a reference to the Postgres service's own variable, a fresh `AUTH_SECRET` from `openssl rand -base64 32`, `AUTH_URL=https://teacher.ioannisbatsios.com`, `AUTH_TRUST_HOST=true`, `ORIGIN_SECRET` from `openssl rand -base64 32`, `EMAIL_SERVER` and `EMAIL_FROM` for the SMTP service, and `ADMIN_EMAIL`. The app refuses to start when `AUTH_TRUST_HOST` is set without `AUTH_URL` and `ORIGIN_SECRET`. Never set `RAILPACK_PRUNE_DEPS`: the pre-deploy command needs the Prisma CLI, which is a dev dependency.
...
5. Email: an SMTP service with a verified sending domain, so magic links arrive. Resend's SMTP endpoint is `smtp://resend:<api key>@smtp.resend.com:465`.
6. The daily backup schedule on the Postgres service.
7. The app keeps sign-in limits in memory, which is right for one instance. If the service is ever scaled to several, move them to a shared store first (D39).
