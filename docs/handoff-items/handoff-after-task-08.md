...
- **Merges to `main` did not deploy on their own for most of Task 08.** The service's source carried a commit pin from when it was created in the dashboard, and `railway environment edit` could not clear it. Reconnecting the repository through the API (`serviceConnect` with the repo and branch `main`) cleared it on 2026-09-06 and started a deployment by itself. If a merge ever fails to deploy again, trigger it with `railway redeploy --service certificate-teacher --from-source --yes` and watch `railway deployment list --service certificate-teacher --limit 1 --json` until it reads SUCCESS.
